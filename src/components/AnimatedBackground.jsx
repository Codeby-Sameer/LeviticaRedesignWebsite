...
const AnimatedBackground = ({ children, className = "" }) => {
  const { isDark } = useTheme();
  const particles = Array.from({ length: 15 }, (_, i) => i); // Reduced particles for light theme
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      ...
      <div className={`absolute inset-0 transition-all duration-500 ${
        ...
        {/* Floating Particles - Reduced opacity in light theme */}
        {particles.map((particle) => (
          <motion.div
            key={particle}
        ...
        {/* Grid Pattern - More subtle in light theme */}
        <div className={`absolute inset-0 bg-[size:50px_50px] ${
          isDark 
            ? 'bg-[linear-gradient(rgba(59,130,246,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.03)_1px,transparent_1px)]'
            : 'bg-[linear-gradient(rgba(37,99,235,0.05)_1px,transparent_1px),linear-gradient(90deg,rgba(37,99,235,0.05)_1px,transparent_1px)] opacity-50'
        }`} />
        
        {/* Gradient Orbs - More subtle in light theme */}
        <motion.div
          className={`absolute top-1/4 left-1/4 w-96 h-96 rounded-full blur-3xl ${
            isDark 
              ? 'bg-blue-500/10' 
        ...
        <motion.div
          ...
          animate={{
            scale: [1.2, 1, 1.2],
            opacity: isDark ? [0.5, 0.3, 0.5] : [0.2, 0.1, 0.2],
          }}
          transition={{
            duration: 6,
            repeat: Infinity,

...
import { 
  ...
  FaSearch, FaFilter, FaCheck, FaTimes, FaArrowRight
} from 'react-icons/fa';
...
const JobManagement = () => {
  ...
  useEffect(() => {
    ...
    const jobsData = storedJobs ? JSON.parse(storedJobs) : initialCareersData;
    setJobs(jobsData);
      ...
      <div className="space-y-4">
        {filteredJobs.length === 0 ? (
          <div className={`text-center py-12 border-2 border-dashed rounded-xl ${
            ...
            <FaSearch className={`w-16 h-16 mx-auto mb-4 ${isDark ? 'text-gray-600' : 'text-gray-400'}`} />
            <h3 className={`text-lg font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
              No jobs found
            </h3>
            <p className={`mb-4 text-sm sm:text-base ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {searchTerm || statusFilter !== 'all' ? 'Try adjusting your search or filters' : 'Create your first job posting'}
            </p>
            <Link
              to="/admin/jobs/new"
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  <div className="flex-1 min-w-0"> {/* Added min-w-0 to prevent overflow */}
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                      <h3 className={`text-lg sm:text-xl font-semibold truncate ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        {job.title}
                      </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm mb-3">
                      ...
                      <span className={`hidden sm:flex items-center ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        <span className="font-medium mr-1">Type:</span> {job.type}
                      </span>
                      <span className={`hidden md:flex items-center ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        <span className="font-medium mr-1">Salary:</span> {job.salary}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-2 pt-4 sm:pt-0 border-t sm:border-t-0 border-slate-700/30">
                    <div className="flex items-center gap-1">
                      ...
                      >
                        {job.isActive ? <FaEyeSlash className="w-4 h-4" /> : <FaEye className="w-4 h-4" />}
                      </button>
                    ...
                    <ul className="space-y-1">
                      {job.requirements.slice(0, 3).map((req, idx) => (
                        <li key={idx} className="flex items-start text-xs">
                          <FaCheck className={`w-3 h-3 mr-2 mt-0.5 flex-shrink-0 ${isDark ? 'text-green-400' : 'text-green-600'}`} />
                          <span className={`line-clamp-1 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                            {req}
                          </span>
                        </li>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      {job.benefits.length > 3 && (
                        <span className={`px-2 py-1 rounded-full text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>
                          +{job.benefits.length - 3}
                        </span>
                      )}
                    </div>
